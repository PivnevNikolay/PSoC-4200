...
//массив сравнения 100 % всегда должны быть разными иначе возникают коллизии
//недолжно быть повтарений особенно в первых симвалах первого и второго массива
...
*/
 ...
 /*LED_Write(~LED_Read());
   CyDelay(1200);
   нет влияния на код задержка отрабатывает и если код соответствует, то
...
*/
    if (count_comparison==5){
        ...
        {
        start_count++;
        UART_PutChar(rec_array[z]);//смотрим что в буффере
        }
        else break;
        } 
        ...
        {
        stopp_count++;
        UART_PutChar(rec_array[v]);//смотрим что в буффере
        }
        else break;
        }
//------------------------------------------------------//    
